...
function rslogin(){
	...
	fetch(url, { method: 'Post' ,
				...
				// fetch에서 헤더의 속성값을 넣는 속성 headers
				// json타입이라는 것을 인지시켜주기 위함
				// => POST 요청에서는 반드시 headers 형식 작성 
                //    (JSON 포맷을 사용함을 알려줘야함)
				// 첫인자에 url, 두번째 인자에 body에 들어갈 데이터들, 세번째 인자에 헤더 속성 값
				// JSON.stringify 함수 : JSON 데이터를 JAVA 의 객체 형태로 만들어주는 메서드
				// 위 메서드에서는 response가 리턴됨. 그 response 객체를 밑에 then 에서 받음
	}).then( response => {
			// ** then 1 단계
			...
			// => response.ok 이후 body 를 읽어야하는데 그 방식중 text()를 사용함(서버에서 Text형식으로 보냈으니)
			// 	  (Type 별로 Body-reading Method를 적용함 )
	}).then( responseData => {
		    // ** then 2 단계
       		// => 1단계에서 return 한 Data 처리
			alert(`** response Date => ${responseData}`)
			location.reload();
	}).catch( err => {
...
function rsloginjj(){
	...
	fetch(url, { method: 'Post' ,
			...
			alert(`** response Date: id=${ responseData.id }, name=${responseData.username} `)
			location.reload();
	}).catch( err => {
		console.error(`** Error => ${ err.message }`);
		if(err.message=='502') alert('id 또는 password 오류 입니다!! 다시하세요.');
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// 2. Axios Login
// => 라이브러리 추가 (CDN 으로..   axTestForm.jsp 에 넣어두었음)
...
function axiloginjj(){
	let url="/rest/rsloginjj"	// 어짜피 json에서 json으로 response하는 메서드는 서버에 있기 때문에 활용
