...
// 2) IOC/DI
// => Speaker 전달 : 맴버변수를 통해 직접 주입
// => @Autowired 
//    -> 자동 주입, 맴버 변수에다 적용
//    -> Only 전달만 함 :  생성문 " ~ = new Speaker()" 에서 = 의 역할을 함
//    -> 그러므로 반드시 미리 생성이 되어있어야 함(@Component 나 Bean을 통해서)
// => @Autowired의 실행 규칙
...
//    -> 맴버변수에 적용할 때는 1:1 이기 때문에 맴버 마다 @Autowired 를 붙여줘야 함
//    -> sstv 에서 생성한 Speaker 인스턴스는 sstv에서 사용하는 전용 인스턴스라 인지하고 구별하고 공급하지 않음
// => .xml 문서에서 Speaker 인스턴스를 bean태그를 통해서 미리 생성해두어도 사용가능함
//@Component("lgtv")
