...
=> EL 출력 : ${"~~ Hello EL, 표현언어"}
	-> Java변수 출력 : \${"EL로 Java변수출력 => " + name} : 500발생 +연산때문에
...
\${ 5 eq 2 } = ${ 5 eq 2 }
\${ 5 != 2 } = ${ 5 != 2 }
<%-- \${ 5 ne 2 } = ${ 5 ne 2 } -> 주의! editor 상에서 오류만 뜸, 실행에선 오류가 없음 --%>
...
=> 조건(삼항)식
\${ 5 > 2 ? '오' : '이' } = ${ 5 > 2 ? '오' : '이' } ->  EL 태그 내부에서는 '' 문자열 구분자 싱글쿼터도 가능함
...
=> \&{ empty_name }: ${ empty name }
   -> empty_ 원래는 _ 없음 연산자로 인식하기 때문에 넣은 구분자임
   -> 자바변수는 출력하지 않음, JSTL로 정의한 변수는 출력
   -> JSTL에 정의된 변수가 없으면 Attribute 에 정의 되어 있는 속성명을 찾게됨  
   -> name 의 값이 없음을 확인
