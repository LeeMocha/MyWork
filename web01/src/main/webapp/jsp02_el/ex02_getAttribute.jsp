...
1) 동일한 속성(Attribute)명을 모든 영역에 정의
	...
	<b>
	\${ name } => ${ name }	-> pageContext 객체에서 부터 먼저 찾게됨.
	=> 가장 가까운(Scope들의 좁은 범위) 객체에서 부터 찾게됨
	=> 동일한 속성명을 사용한 경우에 모두 구별해서 출력하기 위해서는 속성명(해당 객체)을 붙여주면 됨.
	=> 위의 경우엔 속성명(앞에 영역_Scope 객체가)이 생략된것
...
3.2) EL
 \${ requestScope.num1 + requestScopenum2 } = ${ requestScope.num1 + requestScope.num2 }

3.3) Parameter Test : Java
=> 쿼리스트링으로 Test : http://localhost:8080/web01/jsp02_el/ex02_getAttribute.jsp?num1=200&num2=789
<%
